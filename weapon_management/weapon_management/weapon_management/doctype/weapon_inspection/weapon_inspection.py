...
class WeaponInspection(Document):
    def before_save(self):
        if self.result == "Fail":
            weaponRFID = self.weapon_rfid
            frappe.db.set_value("Weapon In Details", {"rfid_tag": weaponRFID}, {"status": "Failed in Inspection"})
...
def get_weapon_details(rfid):
    weaponDetails = frappe.get_value("Weapon In Details",{"status":["in",["Available","Failed in Test Fire","Failed in Servicing"]],"rfid_tag":rfid},

...
def execute(filters=None):
    ...
    query = """
        WITH max_dates AS (
            SELECT ammunition_rfid, MAX(creation) AS max_date
            FROM "tabWeapon and Ammunition Issue"
            GROUP BY ammunition_rfid
        )
        SELECT
            taid.rfid_tag AS "RFID",
            tum.unit_location AS "Unit Location",
        ...
        INNER JOIN
            "tabAmmunition Category Master" tacm ON taid.ammunition_category = tacm.name
        LEFT JOIN 
            max_dates md ON taid.rfid_tag = md.ammunition_rfid
        LEFT JOIN 
            "tabWeapon and Ammunition Issue" twaai ON taid.rfid_tag = twaai.ammunition_rfid AND twaai.creation = md.max_date
        WHERE 1=1
    """

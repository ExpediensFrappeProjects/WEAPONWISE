...
# Keeping this block of code as basic reference/ don't delete it 
# /////////////////////////////////////////////////////////
# import frappe
...
def execute(filters=None):
    ...
    query = f"""
    WITH max_dates AS (
        SELECT weapon_rfid, MAX(date_and_time) AS max_date
        FROM "tabWeapon and Ammunition Issue"
        GROUP BY weapon_rfid
    )
    SELECT 
        twid.rfid_tag as "Weapon RFID",
        twid.weapon_category as "Weapon Category",
        ...
        twid.serial_number as "Weapon Serial Number",
        twid.butt_number as "Weapon Butt Number",
        md.max_date as "Last Issued Date",
        twaai.person_name as "Last Issued To",
        twid.date_acquired as "Acquired Date"
    FROM "tabWeapon In Details" twid
    INNER JOIN "tabUnit Master" tum ON twid.unit_location = tum.name
    LEFT JOIN max_dates md ON twid.rfid_tag = md.weapon_rfid
    LEFT JOIN "tabWeapon and Ammunition Issue" twaai ON twid.rfid_tag = twaai.weapon_rfid AND twaai.date_and_time = md.max_date
    """
